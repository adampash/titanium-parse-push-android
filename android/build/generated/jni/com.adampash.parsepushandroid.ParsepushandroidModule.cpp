...
#define TAG "ParsepushandroidModule"

using namespace v8;

		namespace com {
		namespace adampash {
		namespace parsepushandroid {


Persistent<FunctionTemplate> ParsepushandroidModule::proxyTemplate = Persistent<FunctionTemplate>();
jclass ParsepushandroidModule::javaClass = NULL;

ParsepushandroidModule::ParsepushandroidModule(jobject javaObject) : titanium::Proxy(javaObject)
{
}

void ParsepushandroidModule::bindProxy(Handle<Object> exports)
{
	...
	Handle<String> nameSymbol = String::NewSymbol("Parsepushandroid");
...
void ParsepushandroidModule::dispose()
{
	LOGD(TAG, "dispose()");
	if (!proxyTemplate.IsEmpty()) {
...
Handle<FunctionTemplate> ParsepushandroidModule::getProxyTemplate()
{
	if (!proxyTemplate.IsEmpty()) {
	...
	javaClass = titanium::JNIUtil::findClass("com/adampash/parsepushandroid/ParsepushandroidModule");
	HandleScope scope;
...
, javaClass, nameSymbol);
	...
	proxyTemplate->Set(titanium::Proxy::inheritSymbol,
		FunctionTemplate::New(titanium::Proxy::inherit<ParsepushandroidModule>)->GetFunction());
	...
	instanceTemplate->SetAccessor(String::NewSymbol("exampleProp"),
			ParsepushandroidModule::getter_exampleProp
			, ParsepushandroidModule::setter_exampleProp
...
// Methods --------------------------------------------------------------------
Handle<Value> ParsepushandroidModule::subscribe(const Arguments& args)
{
	LOGD(TAG, "subscribe()");
	HandleScope scope;
...
Handle<Value> ParsepushandroidModule::setup(const Arguments& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(ParsepushandroidModule::javaClass, "setup", "(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/Boolean;");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'setup' with signature '(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/Boolean;'";
			LOGE(TAG, error);
				return titanium::JSException::Error(error);
		}
	}
	...
	if (args.Length() < 2) {
		char errorStringBuffer[100];
		sprintf(errorStringBuffer, "setup: Invalid number of arguments. Expected 2 but got %d", args.Length());
		return ThrowException(Exception::Error(String::New(errorStringBuffer)));
	}
...
Handle<Value> ParsepushandroidModule::example(const Arguments& args)
{
	LOGD(TAG, "example()");
	HandleScope scope;
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(ParsepushandroidModule::javaClass, "example", "()Ljava/lang/String;");
		if (!methodID) {
...
Handle<Value> ParsepushandroidModule::getter_exampleProp(Local<String> property, const AccessorInfo& info)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(ParsepushandroidModule::javaClass, "getExampleProp", "()Ljava/lang/String;");
		if (!methodID) {
...
void ParsepushandroidModule::setter_exampleProp(Local<String> property, Local<Value> value, const AccessorInfo& info)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(ParsepushandroidModule::javaClass, "setExampleProp", "(Ljava/lang/String;)V");
		if (!methodID) {
